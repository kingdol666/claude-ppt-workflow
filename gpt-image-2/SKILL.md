...
> **常见错误：** `generate.js` 的输出路径参数是 `--image`，**不是** `--output`。`--output` 是无效参数，会导致 `Unknown option` 错误（exit 1）。`--output-format` 和 `--output-compression` 仅控制格式/压缩，与控制输出路径无关。
...
node skills/gpt-image-2/scripts/generate.js \
  ...
  --size 1024x1024 \
  --quality high \
  --image garden-gpt-image-2/image/otter.png
```
...
- `study-style-scientific-figure.md` — **Study Style 跨学科学术配图** ★（全学科通用：自然科学/生命科学/工程/社科/医学/环境/人文。暖纸白底 + 深蓝 #1B2A4A + 琥珀金 #C8963E，7 种布局变体：并行流程/左右对比/结构机理/数据图表/方法对比/概念框架/时间轴。Nature/Science 顶刊配图品质，学术答辩 PPT 配图首选。**已通过 7 张跨学科配图实战验证**）
...
> 选择策略：CS/CV/ML 论文首选 `method-pipeline-overview` + `qualitative-comparison-grid`；工程 / 能源 / 化工 / 材料方向首选 `study-style-scientific-figure`（5 种变体覆盖大部分场景）+ `mechanism-diagram` + `multi-condition-comparison`；投稿期刊摘要图用 `graphical-abstract`；答辩 PPT 首页用 `research-overview-poster`。
