...
**规则 4：图片用占位符 div，禁止用 `<img>` 标签**

> **这是最容易踩的坑。** `<img>` 标签的 `src` 路径在 Windows 上会因 Playwright → `file://` → pptxgenjs 的路径解析链而损坏（最终变成 `C:\C:\Users\...` 双重盘符），导致图片无法嵌入 PPTX。
>
> **唯一正确的方式**：HTML 中用 `<div class="placeholder" id="img-*">` 占位，由 `build.js` 在组装阶段通过 `slide.addImage()` 用本地文件系统路径插入图片。

```html
<!-- ❌ 错误：<img> 标签在 Windows 上路径必定损坏 -->
<img src="c:/Users/xxx/image.png" style="position: absolute; ..." />
<!-- Playwright → file:///C:/Users/... → html2pptx 去file:// → /C:/Users/... -->
<!-- pptxgenjs → C:\C:\Users\... (前面 C: 被当作相对路径) → 最终 C:\C:\Users\... -->

<!-- ❌ 错误：background-image 同样不可用 -->
...
```javascript
// html2pptx 解析 HTML 后返回 placeholders 数组，每个包含 { id, x, y, w, h }
const { slide, placeholders } = await html2pptx(htmlPath, pres);

for (const ph of placeholders) {
  const imagePath = path.join(IMAGE_DIR, IMAGE_MAP[ph.id]);
  slide.addImage({
    path: imagePath,    // 本地文件系统路径（正斜杠）
    x: ph.x,            // 英寸，html2pptx 已从 pt 转换
    y: ph.y,
    w: ph.w,
    h: ph.h,
  });
...
**关键约束：**
- 占位符 div 必须有 `class="placeholder"` 和唯一的 `id`（如 `id="img-hero"`）
- 占位符 div 的 CSS 尺寸即最终图片在 PPTX 中的显示尺寸
- 不要在占位符 div 内放任何文字或子元素
- `IMAGE_MAP` 将占位符 id 映射到实际图片文件名
...
<body>
  ...
  <!-- 图片：用占位符 div（build.js 在组装时通过 slide.addImage 插入实际图片）-->
  <div class="placeholder" id="img-side" style="position: absolute; right: 48pt; top: 80pt; width: 240pt; height: 180pt;"></div>
...
| `Background images on DIV elements are not supported` | div 用了 background-image | 改为占位符 div `<div class="placeholder">` |
...
| 图片在 PPTX 中不显示 / 路径损坏 | HTML 中用了 `<img>` 标签 | 改为占位符 div `<div class="placeholder" id="img-*">`，由 build.js 插入 |
